...
% T helper cells
BetaNT=pars(32+12*N);
DeltaNT=pars(33+12*N);
RhoAT=pars(34+12*N);
BetaAT=pars(35+12*N);
% Initial conditions as parameters in the differential equations:
ID0=pars(36+12*N); %#ok<NASGU>
cp0=pars(37+12*N);
ME0=pars((38+12*N):(43+12*N));
NT0=pars((44+12*N):(43+13*N));
MT0=pars((44+13*N):(43+14*N));
...
NT=y((32+13*N):(31+14*N));
% AT_N	activated helper T cells
AT_N=y((32+14*N):(31+15*N));
% AT_M	activated helper T cells
AT_M=y((32+15*N):(31+16*N));
% MT, memory helper T cells
MT=y((32+16*N):(31+17*N));
% FT, functional helper T cells
FT=y((32+17*N):(31+18*N)); %#ok<NASGU>
...
dydt((32+13*N):(31+14*N),1)=BetaNT*(NT0-NT)-DeltaNT*D_N.*NT;

%AT_N, y((35+14*N):(34+15*N)),	activated helper T cells derived from NT,  cells
dydt((32+14*N):(31+15*N),1)=DeltaNT*D_N.*NT+RhoAT.*E_N.*AT_N-BetaAT*AT_N;
